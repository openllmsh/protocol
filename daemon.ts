...
export const DaemonStatus = S.Struct({
  ...
  /** Whether this daemon can host device chat sessions (PTY — POSIX Bun
   *  terminal, or BridgeSessions session-worker when `bs1` is advertised).
   *  False on win32 until a ConPTY worker ships. Absent on daemons too old
   *  to report it — the dashboard then hides the device variant for this box. */
  pty_supported: S.optional(S.Boolean),
  /** Which host PTY backend this daemon will use for new session spawns.
   *  `bridgesessions` = named session-worker; `bun` = in-process `Bun.Terminal`.
   *  Absent on daemons too old to report it. */
  pty_backend: S.optional(S.Literal("bun", "bridgesessions", "conpty")),
